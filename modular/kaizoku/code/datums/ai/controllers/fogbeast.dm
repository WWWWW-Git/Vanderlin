...
/datum/ai_controller/fogbeast/TryPossessPawn(atom/new_pawn)
	. = ..()
	RegisterSignal(new_pawn, COMSIG_ANIMAL_TAMED, PROC_REF(on_user_tamed))

/datum/ai_controller/fogbeast/UnpossessPawn(destroy)
	UnregisterSignal(pawn, COMSIG_ANIMAL_TAMED)
	. = ..()

/datum/ai_controller/fogbeast/proc/on_user_tamed()
	movement_delay = 0.3 SECONDS
	set_blackboard_key(BB_BASIC_MOB_FLEEING, FALSE)
	idle_behavior = null

/datum/ai_controller/fogbeast_foal
