...
/datum/preferences
	...
	var/static/datum/patron/default_patron = /datum/patron/abyssanctum/purifier
...
/datum/preferences/proc/ShowChoices(mob/user, tabchoice)
	...
	dat += "</td>"
	dat += "<td style='width:33%;text-align:right'></td>"
	dat += "</tr>"
	...
	dat += "<BR>"
	dat += "<b>Species:</b> <a href='?_src_=prefs;preference=species;task=input'>[src.pref_species.name]</a>[spec_check() ? "" : " (!)"]<BR>"
	...
	if(AGE_IMMORTAL in src.pref_species.possible_ages)
	...
	ensure_defaults_for_species()

	var/use_skintones = S.use_skintones
	if(use_skintones)
		var/skin_tone_wording = S.skin_tone_wording
	...
	if(length(S.descriptor_choices))
		dat += "<br><b>Descriptors:</b> <a href='?_src_=prefs;preference=descriptors;task=menu'>Change</a>"

	// ----- Culture -----
	var/datum/culture/C = null
	if(culture)
		C = get_culture(culture)

	var/culture_display = C ? C.name : "None"

	dat += "<br><b>Culture:</b> <a href='?_src_=prefs;preference=culture;task=menu'>[culture_display]</a><br>"

	// ----- Body Type (added) -----
	var/datum/body_type/BT_disp = null
	if(body_type)
		BT_disp = get_body_type(body_type)

	var/bodytype_display = BT_disp ? BT_disp.name : "None"
	dat += "<b>Body Type:</b> <a href='?_src_=prefs;preference=bodytype;task=menu'>[bodytype_display]</a><br>"
...
/datum/preferences/proc/process_link(mob/user, list/href_list)
	...
	if(href_list["preference"] == "job")
	...
	else if(href_list["preference"] == "culture" && href_list["task"] == "menu")
		var/datum/species/S = pref_species
		var/list/allowed = S?.get_available_cultures()
		if(!islist(allowed) || !allowed.len)
			to_chat(user, "No cultures available for this race.")
			ShowChoices(user)
			return

		if(isnull(culture) || !(culture in allowed))
			culture = allowed[1]

		var/list/named = list()
		var/default_display = null
		...
		var/choice = browser_input_list(user, "CHOOSE YOUR LACRIMANT'S CULTURE", "THE CURRENTS HAS SHAPED YOU", named, default_display)
		if(choice)
			var/new_id = named[choice]
			if(new_id && (new_id in allowed))
				var/datum/culture/C = get_culture(new_id)
				if(C)
					culture = new_id
					to_chat(user, "Culture set to <b>[C.name]</b> — [C.description]")
				else
					to_chat(user, "<span class='warning'>Invalid culture selected.</span>")
		ShowChoices(user)
		return

	else if(href_list["preference"] == "bodytype" && href_list["task"] == "menu")
		var/datum/species/S = pref_species
		var/list/allowed = S?.get_available_body_types()
		if(!islist(allowed) || !allowed.len)
			to_chat(user, "No body types available for this race.")
			ShowChoices(user)
			return

		if(isnull(body_type) || !(body_type in allowed))
			body_type = allowed[1]
		...
		var/choice = browser_input_list(user, "CHOOSE YOUR LACRIMANT'S BODY TYPE", "THE VESSEL HWEN BY STORMS", named, default_display)
		if(choice)
			var/new_id = named[choice]
			if(new_id && (new_id in allowed))
				var/datum/body_type/BT = get_body_type(new_id)
				if(BT)
					body_type = new_id
					to_chat(user, "Body type set to [BT.name]")
				else
					to_chat(user, "<span class='warning'>Invalid body type selected.</span>")
		ShowChoices(user)
		return

	else if(href_list["preference"] == "culture" && href_list["task"] == "set")
		var/sel_culture = href_list["culture"]
		var/datum/species/S = pref_species
		var/list/allowed = S?.get_available_cultures()
		if(allowed && (sel_culture in allowed) && get_culture(sel_culture))
			culture = sel_culture
			to_chat(user, "Culture set to [culture].")
		else
			to_chat(user, "Invalid culture selected.")
		ShowChoices(user)
		return

	else if(href_list["preference"] == "bodytype" && href_list["task"] == "set")
		var/sel_body = href_list["body_type"]
		var/datum/species/S = pref_species
		var/list/allowed = S?.get_available_body_types()
		if(allowed && (sel_body in allowed) && get_body_type(sel_body))
			body_type = sel_body
			var/datum/body_type/BT = get_body_type(sel_body)
			to_chat(user, "Body type set to [BT?.name || body_type].")
		else
			to_chat(user, "Invalid body type selected.")
		ShowChoices(user)
		return
	...
	switch(href_list["task"])
		...
		if("input")
			...
			switch(href_list["preference"])
				if("name")
					var/new_name = browser_input_text(user, "DECIDE YOUR LACRIMANT'S IDENTITY", "THE DEEP SHALL REMEMBER", real_name, MAX_NAME_LEN, encode = FALSE)
					if(new_name)
				...
				if("age")
					var/new_age = browser_input_list(user, "SELECT YOUR LACRIMANT'S AGE", "MEASURED BY SCARS", pref_species.possible_ages, age)
					if(new_age)
						age = new_age
						ResetJobs(user)
				if ("pronouns")
					...
					var/pronouns_input = browser_input_list(user, "CHOOSE HOW MORTALS REFER TO YOUR LACRIMANT", "LET THE WORLD ADRESS YOU TRUE", allowed_pronouns)
					if(pronouns_input)
				...
				if ("voicetype")
					...
					var/voicetype_input = browser_input_list(user, "CHOOSE YOUR LACRIMANT'S VOICE TYPE", "LET YOUR ECHOES CUT THROUGH FOG", allowed_voices)
					if(voicetype_input)
				...
				if("faith")
					...
					var/faith_input = browser_input_list(user, "SELECT YOUR LACRIMANT'S BELIEF", "WHOSE RITES GUARD YOUR SOUL?", faiths_named, "\The [selected_patron.associated_faith::name]")
					if(faith_input)
				...
				if("patron")
					...
					var/god_input = browser_input_list(user, "SELECT YOUR LACRIMANT'S PATRON GOD", uppertext("\The [current_faith.name]"), patrons_named, selected_patron)
					if(god_input)
				...
				if("voice")
					var/new_voice = input(user, "SELECT YOUR LACRIMANT'S VOICE COLOR", "THE THROAT, SHADE OF INTENT","#"+voice_color) as color|null
					if(new_voice)
				...
				if("species")
					...
					var/result = browser_input_list(user, "SELECT YOUR LACRIMANT'S PEOPLE:", "KIN OF FOG, SALT OR STONE", selectable, pref_species)
				...
				if("charflaw")
					...
					var/result = browser_input_list(user, "SELECT YOUR LACRIMANT'S FLAW", "NO SOUL IS SHORELESS", flawslist, FALSE)
					if(result)
				...
				if("s_tone")
					...
					var/new_s_tone = browser_input_list(user, "CHOOSE YOUR LACRIMANT'S [uppertext(pref_species.skin_tone_wording)]", "PALLETE OF FLESH", listy)
					if(new_s_tone)
				...
				if("selected_accent")
					...
					var/accent
					if(patreon)
						accent = browser_input_list(user, "CHOOSE YOUR LACRIMANT'S ACCENT", "HARBOUR TONGUE FOR THE HIGH MIST", GLOB.accent_list, selected_accent)
						if(accent)
							selected_accent = accent
					else if(change_accent)
						accent = browser_input_list(user, "CHOOSE YOUR LACRIMANT'S ACCENT", "HARBOUR TONGUE FOR THE HIGH MIST", pref_species.multiple_accents, selected_accent)
						if(accent)
			...
			switch(href_list["preference"])
				...
				if("family")
					...
					var/new_family = browser_input_list(user, "SELECT YOUR LACRIMANT'S BOND", "NAMES THAT STAND AGAINST STORMS", famtree_options_list, family)
				...
				if("setspouse")
					var/newspouse = browser_input_text(user, "INPUT THE IDENTITY OF ANOTHER LACRIMANT", "TILL THE DEPTHS TAKES BOTH")
					if(newspouse)
						setspouse = newspouse
					else
						setspouse = null
				...
				if("gender_choice")
					...
					if(pronouns == THEY_THEM || pronouns == IT_ITS)
						...
						var/new_gender_choice  = browser_input_list(user, "SELECT YOUR LACRIMANT'S PREFERENCE", "WHERE YOUR COMPASS POINTS, HEART LEANS LIKE MAST", gender_choice_option_list, gender_choice)
						if(new_gender_choice)
							gender_choice = new_gender_choice
				if("alignment")
					var/new_alignment = browser_input_list(user, "SELECT YOUR LACRIMANT'S MORALITY", "ORDER, CHAOS, OR THE LONG CURRENT", ALL_ALIGNMENTS_LIST, alignment)
					if(new_alignment)
						...
						if(S)
							for(var/i=1, i<=max_save_slots, i++)
								...
								if(!name)
									name = "Slot[i]"
								choices[name] = i
					var/choice = browser_input_list(user, "WHO IS YOUR LACRIMANT?", "WHY DOES THE SEA REMEMBER YOU?", choices, real_name)
					if(choice)
...
/datum/preferences/proc/apply_prefs_to(mob/living/carbon/human/character, icon_updates = TRUE)
	...
	// Body Type resolve (player or NPC fallback)
	var/datum/body_type/BT = get_body_type(body_type)
	if(!BT)
		// Fallback for NPCs/unspecified: first allowed body type of the species
		var/datum/species/S0 = character.dna?.species
		if(S0 && length(S0.allowed_body_types))
			BT = get_body_type(S0.allowed_body_types[1])
	if(BT)
		var/datum/species/S = character.dna?.species
		if(S)
			S.limbs_icon_m = BT.limbs_icon_m
			S.limbs_icon_f = BT.limbs_icon_f
			S.offset_features_m = BT.offset_features_m
			S.offset_features_f = BT.offset_features_f
			// Stonekeep Edit (optional, only if your BTs define these):
			// S.dam_icon_m = BT.dam_icon_m
			// S.dam_icon_f = BT.dam_icon_f
	...
	/* :V */
	apply_body_type_to(character)

	if(icon_updates)
		character.update_body()
		character.update_body_parts(redraw = TRUE)

/datum/preferences/proc/apply_body_type_to(mob/living/carbon/human/H)
	...
	if(hascall(S, "get_available_body_types"))
		allowed = call(S, "get_available_body_types")()
	else
		allowed = S.allowed_body_types
	...
	// Apply per-gender icon + offsets to the mob fields that overlays read
	if(H.gender == MALE)
		if(BT.limbs_icon_m) H.body_type_limb_icon = BT.limbs_icon_m
		if(istype(BT.offset_features_m, /list)) H.body_type_offset_features = BT.offset_features_m
	else
		if(BT.limbs_icon_f) H.body_type_limb_icon = BT.limbs_icon_f
		if(istype(BT.offset_features_f, /list)) H.body_type_offset_features = BT.offset_features_f
